...
readVisitorData <- function(addDateCols = FALSE, beonicfile = NULL) {
  # Fundacion Juan March visitors
  data <- fread("data/visitors/FJM_visitation.csv", header = TRUE)
...
readQueryExplorerData <- function(filename, colNames, addDateCols = FALSE) {
  # Fundacion Juan March GA (from Google Query Explorer)
...
getMaxCorrelationsTable <- function(gametrics, visitors, tptype, maxlag = 31, tofile = NULL) {
  ...
  ans <- cbind(ans, comax[, 2:3])
  #
  # 6) GA Moving average Log - Visitors Moving average
  if (canApplyLog(gametrics)) {
...
getPlotGA <- function(gadata, gametric, holidays = NULL, showTrend = TRUE, showSH = FALSE, showWE = FALSE, showPH = FALSE, showCS = FALSE, showE = FALSE, ylimits = NULL, logData = FALSE, impDates = TRUE) {
  ...
  xlablimits <- as.Date(c("2016-01-01", "2018-09-30"))
  ylabel <- ""
  if (logData) {
...
plotLagComparison <- function(gametrics, visitors, metric = NULL, dlag = NULL, maxcorrs = NULL, showboth = TRUE, plottitle = "Lag visualization") {
  ...
  if (showboth){
    pvis <- ggplot() +
      ...
      labs(title = "", x = "", y = "Fundacion Juan March")
  } else {
    pvis <- ggplot() +
      #geom_line(data = visitors, aes_string("Date", "visitors"), col = "red", lty = 1) +
...
plotLagComparisonSameScale <- function(gametrics, visitors, metric = NULL, dlag = 0) {
  ...
  # Set same lenght
  gametricslag <- gametricslag[Date >= "2016-01-01" & Date <= "2018-09-30"]
  visitorslag <- visitors[Date >= "2016-01-01" & Date <= "2018-09-30"]
  
  finalplot <- ggplot() +
    geom_line(data = visitorslag, aes(Date, (visitors / mean(visitorslag$visitors))), col = "red", size = 0.5)
  
  if (is.null(metric))
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (Sessions / mean(gametricslag$Sessions))), col = "blue", size = 0.5) + labs(x = "", y = "Sessions / Fundacion Juan March")
  else if (metric == "Sessions")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (Sessions / mean(gametricslag$Sessions))), col = "blue", size = 0.5) + labs(x = "", y = "Sessions / Fundacion Juan March")
  else if (metric == "SessionsNew")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (SessionsNew / mean(gametricslag$SessionsNew))), col = "blue", size = 0.5) + labs(x = "", y = "SessionsNew / Fundacion Juan March")
  else if (metric == "SessionsOld")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (SessionsOld / mean(gametricslag$SessionsOld))), col = "blue", size = 0.5) + labs(x = "", y = "SessionsOld / Fundacion Juan March")
  else if (metric == "SessionsBounce")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (SessionsBounce / mean(gametricslag$SessionsBounce))), col = "blue", size = 0.5) + labs(x = "", y = "SessionsBounce / Fundacion Juan March")
  else if (metric == "SessionsNoBounce")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (SessionsNoBounce / mean(gametricslag$SessionsNoBounce))), col = "blue", size = 0.5) + labs(x = "", y = "SessionsNoBounce / Fundacion Juan March")
  else if (metric == "AvgSessionDuration")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (AvgSessionDuration / mean(gametricslag$AvgSessionDuration))), col = "blue", size = 0.5) + labs(x = "", y = "AvgSessionDuration / Fundacion Juan March")
  else if (metric == "Users")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (Users / mean(gametricslag$Users))), col = "blue", size = 0.5) + labs(x = "", y = "Users / Fundacion Juan March")
  else if (metric == "UsersOld")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (UsersOld / mean(gametricslag$UsersOld))), col = "blue", size = 0.5) + labs(x = "", y = "UsersOld / Fundacion Juan March")
  else if (metric == "Pageviews")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (Pageviews / mean(gametricslag$Pageviews))), col = "blue", size = 0.5) + labs(x = "", y = "Pageviews / Fundacion Juan March")
  else if (metric == "PageviewsSession")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (PageviewsSession / mean(gametricslag$PageviewsSession))), col = "blue", size = 0.5) + labs(x = "", y = "PageviewsSession / Fundacion Juan March")
  else if (metric == "UniquePageviews")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (UniquePageviews / mean(gametricslag$UniquePageviews))), col = "blue", size = 0.5) + labs(x = "", y = "UniquePageviews / Fundacion Juan March")
  else if (metric == "AvgTimeOnPage")
    finalplot <- finalplot + geom_line(data = gametricslag, aes(Date, (AvgTimeOnPage / mean(gametricslag$AvgTimeOnPage))), col = "blue", size = 0.5) + labs(x = "", y = "AvgTimeOnPage / Fundacion Juan March")
